...
const initFlashLoanEmpoweredArbitrage = async (encodedSwaps: string) => {
    ...
    const data = arbitrageContract.methods.initFlashLoanEmpoweredArbitrage(
        PRIMARY_TOKEN_ADDRESS, PRIMARY_TOKEN_INPUT_AMOUNT, encodedSwaps).encodeABI();
    const transaction = {
    ...
    const { rawTransaction } = await web3.eth.accounts.signTransaction(transaction,
        TEST ? ARBITRAGE_MANAGER_ADDRESS : ARBITRAGE_MANAGER_PRIVATE_KEY);
